...
* Авто-закрытие приложения при выходе из Windows
...
* Маломальская функция обработки ошибок
FUNCTION ChekErr
	LPARAMETERS _Error,_Message,_Message1,_Program,_Lineno1
	LOCAL lcErrMessage, lnFH
	lcErrMessage='В программе '+_Program+' произошла ошибка (строка '+TRANSFORM(_Lineno1)+')'+CHR(13)+CHR(10)+;
		'Ошибка N:'+TRANSFORM(_Error)+' '+_Message+CHR(13)+CHR(10)+;
		'Исходный код: '+_Message1
